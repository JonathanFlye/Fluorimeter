/*
    ---------- Open Source Oceanography Data loger
    www.openoceanography.org
    Data logger pour les projet OPEN LAGOON/OSBALOUM/DYSRUP.

    Cet instrument mesure :
    Sortie d'un capteur RS323 : Flourimètre Trilux (Fluo, Turbidité) / TChain (Témpérature et Oxygène à différents niveaux)
...
// definiton for TS232 port (Fluo, Tchain...) sensors and logging
//String fluochain,datafluo,fluoFinal; // TO REMOVE
String rs232_1_chain, data_rs232;  // Generic RS232
...
String rs232_channels = "Temp surf (c);DO surf (umol/l);Temp int1 (C);DO int1 (umol/l);Temp int2 (C);Temp int3 (C);DO bottom (umol/l);Temp bottom (C)"; // pour la TChain
//String rs232_channels = "Chl, Nph, Valim, Vref"; // pour la trilux
...
void setup() {
  ...
  //datafluo.reserve(32); // TO REMOVE
  data_rs232.reserve(256);
  datachain.reserve(300);
...
//     on reveille le GPS et on le lit une seule fois pour le meme batch de mesures
        ...
        delay(500);
        if(!gps.location.isUpdated()) affiche_fix_notok();
    ...
    // Reveillez l'alim de la trilux et la laisser chauffer
      Serial.println("Powering RS232 1 (12V), and let the instrument warm up : ");Serial.print(delay_chauffe);Serial.println(" s");
    ...
    for (int i = 0; i < nbrMes; i++) {
      ...
      delay(delay_lec);
      Serial.println("Start reading RS232 1");
      mes_rs232_1();               // Read chain from RS232 and save it in the strign data_rs232
      Serial.print("Start parsing rs232 :");
      //parse_fluo();
      parse_rs232();
      delay(delay_lec);
      ...
      datachain += data_rs232; datachain += ";";
    ...
    if(mode_screen=1){
        ...
        display.setCursor(5, 70); display.print("RS232 :"); display.print(data_rs232.substring(0,43));
        ...
        display.setCursor(0, 124); display.print("Temp int. : "); display.print(tempint); display.print(" deg C");
...
void affiche_fix_notok(){                 //message de fix not OK
  display.setRotation(3);
  display.fillScreen(GxEPD_WHITE);
...
// Fonctions pour la lecture du port RS232
void mes_rs232_1(){
  ...
  Serial.print("RS232 1 data : "); Serial.println(data_rs232); // juste pour voir la valeur de datafluo
}

void parse_rs232(){
  //----------------- parsing and filtering  data from rs232 sensors ---------------------
  // The code below allow to replace or remove 
  // 

  data_rs232.replace("     ", ";");
  data_rs232.replace("    ", ";");
  data_rs232.replace("V","");
  data_rs232.trim();
  //Serial.print(" First filter "); Serial.println(data_rs232);

  // Check if first value is a ; and remove if so
  if(data_rs232.charAt(0) ==';'){
    //Serial.println("First char is ;, removing");
    data_rs232.remove(0,1);
  }
  if(data_rs232.charAt(data_rs232.length()) ==';'){
    //Serial.println("Last char is ;, removing");
    data_rs232.remove(data_rs232.length(),1);
  }


  Serial.println("--- RS232 Sensor Final :");
  Serial.println(data_rs232);
